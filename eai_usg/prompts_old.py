...
- support_type: "explicit" if directly stated in the source, or
  "reasonable_implication" if it follows directly from the source without
  requiring an additional assumption.

A reasonable implication must follow from the requirement without requiring an
additional assumption.

Possible operational aspects must represent substantive source conditions,
not inferred implementation steps.

Do not create obligations from implementation prerequisites, possible
operational aspects, or unsupported assumptions.

Separate distinct obligations when useful, but do not manufacture additional
requirements or artificially split closely related conditions that belong to
the same substantive obligation.
...
The analysis contains atomic obligations identified from the source requirement.

Generate the EUS according to all rules above.

In addition, provide a traceability map:
- identify the obligation IDs supporting the substantive content of the description;
- identify one or more obligation IDs supporting each work item;
- use zero-based work_item_index values corresponding to the generated work_items list;
- every work item must have at least one supporting obligation;
- an obligation may support multiple EUS elements;
- an EUS element may be supported by multiple obligations.

Traceability must reflect the content actually generated.
Do not introduce new content merely to create a trace link.
Do not invent obligations to justify generated content.
"""
...
A traceability map is provided with the candidate EUS.

Treat the traceability map as evidence to inspect, not as proof that the mappings are correct.

Verify that:
- each work item is genuinely supported by the obligation IDs linked to it;
- the description is supported by the obligations linked to it;
- substantive source obligations are represented in the EUS where appropriate;
- generated substantive content is supported by the source requirement;
- traceability does not conceal unsupported additions.

Use the original source requirement as the ultimate source of truth.

Deterministic traceability checks report structural mapping problems only. Independently assess semantic support.
"""
...
Return a complete traceability map for the revised EUS.

Rebuild the mappings based on the revised content rather than copying the previous mappings mechanically.

Every revised work item must be supported by one or more obligations from the provided analysis.

Do not create new obligations or unsupported content to resolve validation feedback.
"""
